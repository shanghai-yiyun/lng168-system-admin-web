...
export default modelEnhance({
    ...
    subscriptions: {
        setup({dispatch, history}) {
            history.listen(({pathname}) => {
                if (pathname === '/adminBusPur' && !LOADED) {
                    LOADED = true;
                    dispatch({
                        type: 'init'
                    });
    ...
    effects: {
        ...
        * init({payload}, {call, put, select}) {
            const {pageData} = yield select(state => state.adminBusPur);
            yield put({
                type: 'getPageInfo',
                payload: {
